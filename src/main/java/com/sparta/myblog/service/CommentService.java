...
public class CommentService {
	...
	// 2. 댓글 수정하기
	@Transactional
	public ResponseEntity <ApiResponseDto> updateComment (Long commentId, CommentRequestDto requestDto, User user) {

		Comment comment = commentRepository.findById(commentId).orElse(null);

		if (comment == null) {
			log.error("수정할 댓글이 존재하지 않음");
			return ResponseEntity.status(400).body(new ApiResponseDto("수정할 댓글이 존재하지 않습니다.", HttpStatus.BAD_REQUEST.value()));

		} else if (!comment.getUser().getUserId().equals(user.getUserId())){
			log.error("권한 없는 사용자의 접근");
			return ResponseEntity.status(400).body(new ApiResponseDto("작성자만 수정할 수 있습니다.", HttpStatus.BAD_REQUEST.value()));

		}

		comment.update(requestDto.getComment());
		return ResponseEntity.status(200).body(new ApiResponseDto("댓글 수정이 완료되었습니다.", HttpStatus.OK.value()));
